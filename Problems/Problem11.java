...
public class Problem11 {
	...
	public static void main(String[] args) {
		int mat[][] = new int[][] { { 8, 02, 22, 97, 38, 15, 00, 40, 00, 75, 04, 05, 07, 78, 52, 12, 50, 77, 91, 8 },
				{ 49, 49, 99, 40, 17, 81, 18, 57, 60, 87, 17, 40, 98, 43, 69, 48, 04, 56, 62, 00 },
				{ 81, 49, 31, 73, 55, 79, 14, 29, 93, 71, 40, 67, 53, 88, 30, 03, 49, 13, 36, 65 },
				{ 52, 70, 95, 23, 04, 60, 11, 42, 69, 24, 68, 56, 01, 32, 56, 71, 37, 02, 36, 91 },
				{ 22, 31, 16, 71, 51, 67, 63, 89, 41, 92, 36, 54, 22, 40, 40, 28, 66, 33, 13, 80 },
		...
		System.out.println(adjacentProductOf4(mat, 4));
	...
	private static int lowerDiagonal(int[][] mat, int n) {
		...
		for (int i = mat.length - n; i >= 0; i--) {
			for (int j = 0, t = i; j < mat.length - i - (n - 1); j++, t++) {
				...
				if (max < sum)
					max = sum;
				/* uncomment and see the possible array combination 
				 * System.out.println("[" + t + "," + j + "] [" + (t + 1) + "," + (j + 1) + "]["
				 * + (t + 2) + "," + (j + 2) + "][" + (t + 3) + "," + (j + 3) + "]");
				 */
			}
		}
		...
		for (int i = 1; i <= mat.length - n; i++) {
			for (int j = 0, t = i; j < mat.length - i - (n - 1); j++, t++) {
				...
				if (max < sum)
					max = sum;
				/*
				 * System.out.println("[" + j + "," + t + "] [" + (j + 1) + "," + (t + 1) + "]["
				 * + (j + 2) + "," + (t + 2) + "][" + (j + 3) + "," + (t + 3) + "]");
				 */
			}
			// System.out.println();
